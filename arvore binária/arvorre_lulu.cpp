...
void trocar_pais(No *pai,No* filho1,No*filhoTroca){
...
bool esquerda_direita(No *pai,No *filho){
  if(filho->num<pai->num){
    return true;
  }else{
    return false;
  }
...
void swap_node(No **raiz,No *node1, No *node2){
 if (*raiz == NULL) {
        return;
    }
    ...
    No* novopai1=buscar_pai(*raiz,node1->num);
    No* novopai2=buscar_pai(*raiz,node2->num);
    ...
    if(node1==*raiz){
      if(esquerda_direita(novopai1,node1)==0){
        
      }if(esquerda_direita(novopai2,node2)==1){
...
int main() {
  ...
  while (opc != 0) {
    ...
    switch (opc) {
      ...
      case 4:
        //remover_raiz(&root);
        printf("informe dois numeros para trocar");
        scanf("%d",&num1);
         printf("informe dois numeros para trocar");
        scanf("%d",&num2);

        // swap_node(root,num1,num2);
        break;
      case 5:
